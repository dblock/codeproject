...
public:
	...
	BOOL ExtractFileW(const CStrW& sw_CabPath, const CStrW& sw_TargetDir, void* pParam = NULL)
	{
		...
		// We MUST call FdiCopy for each part: Part_1.cab,Part_2.cab,Part_3.cab,Part_4.cab 
...
private:
	...
	{ 
		...
		if (s32_Written > 0 && mk_CurrentFile.h_File == fd && mk_Callbacks.f_OnProgressInfo)
		{
			...
			if (abs(s32_Now - mk_CurrentFile.s32_LastTick) > 200)
			{
	...
	INT_PTR FdiCallback(FDINOTIFICATIONTYPE fdint, PFDINOTIFICATION pfdin)
	{
		...
		switch (fdint)
		{
			case fdintCABINET_INFO:
			{
				...
				// But if this callback is called after a fdintNEXT_CABINET do NOT store the next CAB
				// because fdiCopy has not yet been called for the CAB file that is currently in msw_NextCab!
